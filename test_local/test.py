...
tl.files.exists_or_mkdir("E:/Attribute2Sketch/test_local_da8/hd_real_image")
tl.files.exists_or_mkdir("E:/Attribute2Sketch/test_local_da8/hd_fake_image")
tl.files.exists_or_mkdir("E:/Attribute2Sketch/test_local_da8/low_fake_image")
tl.files.exists_or_mkdir("E:/Attribute2Sketch/test_local_da8/low_real_image")
save_dir = "E:/Attribute2Sketch/output_local_da8/checkpoint" # 模型加载路径
...
def main_test():
    ...
    net_fake_image,_ = generator(t_z,net_capt.outputs,is_train=False,reuse=False,batch_size=batch_size) 
    net_d,disc_fake_image_logits = discriminator(net_fake_image.outputs,net_capt.outputs,is_train=False,reuse=False)
    _,disc_real_image_logits = discriminator(t_real_image,net_capt.outputs,is_train=False,reuse=True)
    _,disc_mismatch_logits = discriminator(t_real_image,capt_embed(t_wrong_caption).outputs,is_train=False,reuse=True)
    ...
    net_hd_fake_image,_ = hd_generator(t_hd_fake_image,net_capt.outputs,is_train=False,reuse=False,batch_size=batch_size)
    
    net_hd_d,disc_hd_fake_image_logits = hd_discriminator(net_hd_fake_image.outputs,net_capt.outputs,is_train=False,reuse=False)
    _,disc_hd_real_image_logits = hd_discriminator(t_hd_real_image,net_capt.outputs,is_train=False,reuse=True)
    _,disc_hd_mismatch_logits = hd_discriminator(t_hd_real_image,capt_embed(t_wrong_caption).outputs,is_train=False,reuse=True)
    ...
    for epoch in range(0, n_epoch+1):
        ...
        for step in range(278):
            step_time = time.time()
            ## get matched text
            # i = batch_size*step
            # print(i)
            # idexs = [i for i in range(i+batch_size)]
            # idexs = [batch_size*step:batch_size*step+batch_size] ## 每个batch图片和描述对应的序号
            idexs = [step]
            real_idexs = idexs
            #print(idexs)
            ...
            save_images(fake_img_gen,[ni,ni], 'E:/Attribute2Sketch/test_local_da8/low_fake_image/test_{:02d}.png'.format(step+epoch*n_batch_epoch))
            save_images(b_real_images,[ni,ni],'E:/Attribute2Sketch/test_local_da8/low_real_image/test_{:02d}.png'.format(step+epoch*n_batch_epoch))
            save_images(img_gen,[ni,ni], 'E:/Attribute2Sketch/test_local_da8/hd_fake_image/test_{:02d}.png'.format(step+epoch*n_batch_epoch)) 
            save_images(b_hd_real_images,[ni,ni],'E:/Attribute2Sketch/test_local_da8/hd_real_image/test_{:02d}.png'.format(step+epoch*n_batch_epoch))         
